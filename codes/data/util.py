...
def augment(img, hflip=True, rot=True, scale=False):
    # horizontal flip OR rotate
    hflip = hflip and random.random() < 0.5
    rot90 = rot and random.random() < 0.5
    if scale:
        scales = [1.0, 1.2, 1.5]
        sc = scales[random.randint(0, len(scales) - 1)]

    def _augment(name, img):
        if img.ndim == 3:
            if hflip:
                img = img[:, ::-1, :].copy()
            if rot90:
        ...
        if scale:
            ...
            if name == 'GT' or name == 'LR':
                img = cv2.warpAffine(img, M, (w, h), flags=2)
            else:
                img = cv2.warpAffine(img, M, (w, h), flags=0)
            if name == 'blur':
                img = img / sc
        return img

    return {k: _augment(k, v) for k, v in img.items()}
